    ...
    <ol className="flex flex-col">
      {items.map((item, i) => {
        if (item.kind === "block") {
          const { block } = item
          return (
            <li key={`block-${block.id}`} className="relative flex gap-3 pb-4 last:pb-0 sm:pb-5">
              {i < items.length - 1 && <span aria-hidden className="bg-border absolute top-6 bottom-0 left-[47px] w-px" />}
              <span className="text-foreground w-11 shrink-0 pt-2.5 font-mono text-sm font-semibold tabular-nums">
        ...
        const { booking } = item
        return (
          <li key={booking.id} className="relative flex gap-3 pb-4 last:pb-0 sm:pb-5">
            {i < items.length - 1 && <span aria-hidden className="bg-border absolute top-6 bottom-0 left-[47px] w-px" />}
            <span className="text-foreground w-11 shrink-0 pt-2.5 font-mono text-sm font-semibold tabular-nums">
            ...
            />
            <div
              className="bg-card flex flex-1 flex-col gap-3 rounded-lg border py-3 pr-3.5 pl-3 sm:flex-row sm:items-start sm:justify-between"
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-muted-foreground mt-0.5 text-xs">
                  {booking.service.name} · {formatDuration(booking.service.durationMinutes)} · {formatBRL(booking.service.price)}
                </p>
              </div>
